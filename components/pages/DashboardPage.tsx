    ...
    useEffect(() => {
        const generateInsight = async () => {
            ...
            try {
                ...
                const highScores = academicRecords.filter(r => r.score > 85).length;
                const lowScores = academicRecords.filter(r => r.score < 70).length;
                const totalViolationPoints = violations.reduce((acc, v) => acc + v.points, 0);


                const prompt = `
                    Analisis data guru berikut dan berikan wawasan.
                    ...
                    - Total Nilai > 85: ${highScores}
                    - Total Nilai < 70: ${lowScores}
                    - Total Poin Pelanggaran: ${totalViolationPoints}

                    Tugas Anda:
                    1. Identifikasi 1-2 siswa yang berprestasi (nilai tinggi, peningkatan signifikan) untuk diberi pujian. Berikan nama siswa dan alasannya.
                    2. Identifikasi 1-2 siswa yang paling memerlukan perhatian (nilai turun, pelanggaran, sering alpha). Berikan nama siswa dan alasannya.
                    3. Berikan satu saran fokus untuk kelas secara umum (misalnya, materi yang sulit atau pengumuman penting).
                    
                    Format jawaban Anda HARUS dalam bentuk JSON yang sesuai dengan skema yang diberikan. Jika tidak ada siswa yang cocok untuk satu kategori, kembalikan array kosong.
                `;
                ...
                const responseSchema = {
                    ...
                    properties: {
                        to_praise: {
                            ...
                            items: { type: Type.OBJECT, properties: { student_name: { type: Type.STRING }, reason: { type: Type.STRING, description: "Alasan pujian, cth: 'Nilai 100 pada kuis Matematika.'" } } }
                        },
                        needs_attention: {
                            type: Type.ARRAY, description: "Daftar siswa yang memerlukan perhatian.",
                            items: { type: Type.OBJECT, properties: { student_name: { type: Type.STRING }, reason: { type: Type.STRING, description: "Alasan, cth: 'Tingkat kehadiran menurun dan mendapat poin pelanggaran.'" } } }
                        },
                        class_focus: {
